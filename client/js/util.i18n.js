...
const LANGUAGES = {
	en: {
		...
		translations: {
			...
			// [新增-消息留存] Message retention
			// 新增留存功能时集中追加，避免与现有词条交叉改动
			'system.retention_on': 'Messages in this room are kept for {value}',
			'system.retention_off': 'Messages in this room are not stored',
			'system.retention_owner': 'you are the owner and can change this in settings',
			'system.retention_sent': 'Retention change sent',
			'ui.retention': 'Keep messages (minutes)',
			'ui.retention_requires_password': 'A room password is required to keep history',
			'ui.owner_password': 'Owner password',
			'ui.owner_password_hint': 'Without an owner password the retention time can never be changed',
			'settings.retention': 'Message Retention',
			'settings.retention_current': 'This room keeps messages',
			'settings.retention_default': 'Default for new rooms (minutes)',
			'settings.retention_change': "Change this room's retention (minutes)",
			'settings.retention_apply': 'Apply',
			'settings.retention_owner_badge': 'you are the owner',
			'settings.retention_owner_only': 'Only the room owner (whoever knows the owner password) can change this',
			'settings.retention_zero_warning': 'Setting 0 immediately clears all history stored for this room',
			'settings.retention_note': 'A new room\'s retention is decided by the first person who joins it, and can afterwards only be changed by whoever knows the owner password. 0 means nothing is stored. History is encrypted with the room password, so the server only ever holds ciphertext.',
			'retention.off': 'Not stored',
			'retention.minutes': '{n} minutes',
			'retention.hours': '{n} hours',
			'retention.hours_minutes': '{h} h {m} min',
			'help.retention': '⏱️ Optional Retention',
			'help.retention_desc': 'By default nothing is stored and messages vanish when everyone leaves. If you set a retention time, history is kept for that long and encrypted with the room password — the server still cannot read it. The first person to join decides this, and afterwards only whoever knows the owner password can change it (or set it to 0 to erase the history).',
					// Help page
	...
	zh: {
		...
		translations: {
			...
			// [新增-消息留存] 消息留存
			// 新增留存功能时集中追加，避免与现有词条交叉改动
			'system.retention_on': '本房间的消息将保留 {value}',
			'system.retention_off': '本房间不保存任何消息',
			'system.retention_owner': '你是房主，可在设置中修改',
			'system.retention_sent': '留存时长修改已发送',
			'ui.retention': '消息保留时长（分钟）',
			'ui.retention_requires_password': '需要设置房间密码才能保留历史消息',
			'ui.owner_password': '房主管理密码',
			'ui.owner_password_hint': '不设置管理密码，留存时长之后将无法修改',
			'settings.retention': '消息留存',
			'settings.retention_current': '本房间保留消息',
			'settings.retention_default': '新建房间时的默认时长（分钟）',
			'settings.retention_change': '修改本房间留存时长（分钟）',
			'settings.retention_apply': '应用',
			'settings.retention_owner_badge': '你是房主',
			'settings.retention_owner_only': '只有房主（知道房主管理密码的人）可以修改',
			'settings.retention_zero_warning': '设为 0 会立即清除本房间已保存的全部历史',
			'settings.retention_note': '新建房间时，留存时长由第一个进入的人决定；此后只有知道房主管理密码的人可以修改。0 表示不保存任何消息。历史记录使用房间密码加密，服务器只能拿到密文。',
			'retention.off': '不保存',
			'retention.minutes': '{n} 分钟',
			'retention.hours': '{n} 小时',
			'retention.hours_minutes': '{h} 小时 {m} 分钟',
			'help.retention': '⏱️ 可选的消息留存',
			'help.retention_desc': '默认不保存任何消息，所有人离开后消息即消失。若设置了留存时长，历史消息会在该时长内保留，并使用房间密码加密——服务器依然无法读取。该时长由第一个进入房间的人决定，之后只有知道房主管理密码的人可以修改（设为 0 会清除已保存的历史）。',
